...
const removeAcquiredStudyM = 'DELETE FROM study_material_acquired WHERE study_id=$1;';

const removeAllStudyMExchangeR = `DELETE FROM study_material_exchange_request
    WHERE requester=$1 AND study_id_requester=$2;`;

const removeStudyMExchangeRs = `DELETE FROM study_material_exchange_request
                                WHERE exchange_id=$1`;
